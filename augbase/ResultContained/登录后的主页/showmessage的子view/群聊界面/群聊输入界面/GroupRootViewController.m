...
@interface GroupRootViewController ()<UUInputFunctionViewDelegate,UUMessageCellDelegate,UITableViewDataSource,UITableViewDelegate,SendTableResultDele,ShowWebViewDele>

@property (strong, nonatomic) MJRefreshHeaderView *head;
@property (strong, nonatomic) GroupChatModel *chatModel;
@property (nonatomic) NSInteger groupMessTime;
...
@implementation GroupRootViewController{
    ...
    BOOL showBottom;//是否显示了底部view
}

- (void)viewDidLoad {
    [super viewDidLoad];
    NSLog(@"群组的jid====%@",_groupJID);
    [[SetupView ShareInstance]showHUD:self Title:NSLocalizedString(@"一大波战友正涌来...", @"")];
    _groupMessTime = 1;
...
-(void)setupBottomView{
    ...
    ImageAndLabelView *cameraView = [[ImageAndLabelView alloc]initWithFrame:CGRectMake(32, 12, 59, 80)];
    ...
    ImageAndLabelView *labView = [[ImageAndLabelView alloc]initWithFrame:CGRectMake(32+59+(ViewWidth-32*2-59*4)/3, 12, 59, 80)];
    ...
    ImageAndLabelView *diseaseTableView = [[ImageAndLabelView alloc]initWithFrame:CGRectMake(32+59*2+(ViewWidth-32*2-59*4)/3*2, 12, 59, 80)];
    diseaseTableView.titleImageView.image = [UIImage imageNamed:@"form2"];
    diseaseTableView.titleLabel.text = NSLocalizedString(@"大表", @"");
...
#warning 此处应该加入选择的对象jid
    ...
    if([[DBGroupManager ShareInstance] isChatTableExist:[NSString stringWithFormat:@"%@%@",YizhenTableName,_groupJID]]){
        [[DBGroupManager ShareInstance] SetNotReadToRead:[NSString stringWithFormat:@"%@%@",YizhenTableName,_groupJID]];
    }
}

-(void)viewWillAppear:(BOOL)animated{
    [XMPPSupportClass ShareInstance].receiveMessDelegate = self;
    if (_groupTitle == nil||[_groupTitle isEqualToString:@""]) {
...
#warning 此处需要加入从数据库获取数据
    ...
    [[DBGroupManager ShareInstance] creatDatabase:GroupChatDBName];
    [[DBGroupManager ShareInstance] isChatTableExist:[NSString stringWithFormat:@"%@%@",YizhenTableName,_groupJID]];
    [[DBGroupManager ShareInstance] SetNotReadToRead:[NSString stringWithFormat:@"%@%@",YizhenTableName,_groupJID]];
}
...
- (void)todoSomething:(id)sender
{
    ...
    gdv.groupTitle = _groupTitle;
    gdv.groupJID = _groupJID;
    gdv.groupNote = _groupNote;
    [self.navigationController pushViewController:gdv animated:YES];
}

- (void)groupDetail:(id)sender
{
    //先将未到时间执行前的任务取消。
...
#warning 需要加入正常发送情况下的代码,text已经可以了，差音频和图片
    ...
    chatDBItem.toPersonJID = _groupJID;
    ...
    chatDBItem.chatType = 1;//群聊
    chatDBItem.timeStamp = currenttime;
    
    chatDBItem.FromMeOrNot = 0;
    chatDBItem.ReadOrNot = 1;
    
    if ([[XMPPSupportClass ShareInstance] sendMess:chatDBItem toUserJID:_groupJID FromUserJID:userJID]) {
        [self.chatModel addCellFromDB:_groupJID MessNumber:10];
        [self.chatTableView reloadData];
        [self tableViewScrollToBottom];
    }
