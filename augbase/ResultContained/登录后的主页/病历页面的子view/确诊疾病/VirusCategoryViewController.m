...
static NSString *virusCellIndentifyDisease = @"virusCategoryCell";

@interface VirusCategoryViewController ()
...
@implementation VirusCategoryViewController
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    LableCell *cell;
    cell = [collectionView dequeueReusableCellWithReuseIdentifier:virusCellIndentifyDisease forIndexPath:indexPath];
...
-(void)setupView{
    ...
    _virusCategoryDiseaseCollection = [[UICollectionView alloc]initWithFrame:CGRectMake(0, 0, ViewWidth, ViewHeight) collectionViewLayout:lineLayout];
    _virusCategoryDiseaseCollection.dataSource = self;
    _virusCategoryDiseaseCollection.delegate = self;
    _virusCategoryDiseaseCollection.backgroundColor = [UIColor whiteColor];
    [_virusCategoryDiseaseCollection registerClass:[LableCell class]forCellWithReuseIdentifier:virusCellIndentifyDisease];
    [self.view addSubview:_virusCategoryDiseaseCollection];
}
    ...
    [[HttpManager ShareInstance]AFNetGETSupport:[NSString stringWithFormat:@"%@v2/disease/all?uid=%@&token=%@",Baseurl,[[NSUserDefaults standardUserDefaults] objectForKey:@"userUID"],[[NSUserDefaults standardUserDefaults] objectForKey:@"userToken"]] Parameters:nil SucessBlock:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (res == 0) {
            ...
            [[HttpManager ShareInstance]AFNetGETSupport:[NSString stringWithFormat:@"%@und/list?uid=%@&token=%@",Baseurl,[[NSUserDefaults standardUserDefaults] objectForKey:@"userUID"],[[NSUserDefaults standardUserDefaults] objectForKey:@"userToken"]] Parameters:nil SucessBlock:^(AFHTTPRequestOperation *operation, id responseObject) {
                ...
                int res = [[source objectForKey:@"res"] intValue];
                if (res == 0) {
                    NSArray *disListArray = [source objectForKey:@"virusList"];
                    for (NSDictionary *dic in disListArray) {
                        NSInteger index = [[dic objectForKey:@"id"] integerValue]-20;
                        [diseaseConfirmArray replaceObjectAtIndex:index withObject:@1];
                    }
                    [_virusCategoryDiseaseCollection reloadData];
                }else{
...
-(void)viewWillDisappear:(BOOL)animated{
    for (int i=0;i<diseaseConfirmArray.count;i++) {
        if ([diseaseConfirmArray[i] intValue] == 1) {
            if (modifyString == nil) {
                modifyString = [NSString stringWithFormat:@"%d",i+20];
            }else{
                modifyString = [NSString stringWithFormat:@"%@_%d",modifyString,i+20];
            }
        }
    }
    [[HttpManager ShareInstance]AFNetPOSTSupport:[NSString stringWithFormat:@"%@und/edit?uid=%@&token=%@&category=%d&did=%@",Baseurl,[[NSUserDefaults standardUserDefaults] objectForKey:@"userUID"],[[NSUserDefaults standardUserDefaults] objectForKey:@"userToken"],2,modifyString] Parameters:nil ConstructingBodyWithBlock:nil SucessBlock:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSDictionary *source = [NSJSONSerialization JSONObjectWithData:responseObject options:NSJSONReadingMutableLeaves error:nil];
        int res = [[source objectForKey:@"res"] intValue];
        if (res == 0) {
            NSLog(@"修改成功");
            [_virusDele virusCategoryDelegate:YES];
        }
    } FailedBlock:^(AFHTTPRequestOperation *operation, NSError *error) {
